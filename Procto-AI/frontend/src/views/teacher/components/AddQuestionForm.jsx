...
import { useCreateQuestionMutation, useGetExamsQuery, useGetCoursesQuery } from 'src/slices/examApiSlice';
import { toast } from 'react-toastify';

const AddQuestionForm = ({ courseId: propCourseId, selectedExamId: propSelectedExamId }) => {
  ...
  useEffect(() => {
    if (examsData && examsData.length > 0) {
      if (propSelectedExamId && examsData.some(e => e.examId === propSelectedExamId)) {
        setSelectedExamId(propSelectedExamId);
      } else {
        setSelectedExamId(examsData[0].examId);
      }
    }
  }, [examsData, propSelectedExamId]);
  ...
  const handleAddQuestion = async () => {
    ...
    const newQuestionObj = {
      ...
      courseId: propCourseId || selectedCourseId,
    };
