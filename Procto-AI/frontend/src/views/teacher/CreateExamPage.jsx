...
import SelectQuestionsForExam from './components/SelectQuestionsForExam';
...
const CreateExamPage = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      const res = await createExam({ ...values, courseId }).unwrap();
      toast.success('Exam Created successfully');
      formik.resetForm();
      setNewExamId(res.examId);
    } catch (err) {
            ...
            <Card elevation={9} sx={{ p: 4, zIndex: 1, width: '100%', maxWidth: '500px' }}>
              {courseId && (
                <Button
                  variant="outlined"
                  sx={{ mb: 2, width: '100%' }}
                  onClick={() => navigate(`/course-mcq-bank?courseId=${courseId}`)}
                >
                  Go to MCQ Bank for this Course
                </Button>
              )}
              {!newExamId ? (
                <ExamForm
                ...
                <SelectQuestionsForExam courseId={courseId} examId={newExamId} />
              )}
            </Card>
          </Grid>
        </Grid>
