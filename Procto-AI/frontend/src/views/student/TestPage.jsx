...
const TestPage = () => {
  ...
  const handleTestSubmission = async (correctAnswers) => {
    try {
      ...
      // Debug logs
      console.log('Submitting cheatingLogData:', cheatingLogData);
      console.log('Submitting resultLogData:', resultLogData);
      console.log('userInfo:', userInfo);
      console.log('examId:', examId);

      // Try cheating log, but don't block on error
      try {
        await saveCheatingLog(cheatingLogData);
        await saveCheatingLogMutation(cheatingLogData).unwrap();
      } catch (err) {
        console.log('cheating log error (ignored for finish):', err);
      }

      await saveResultLogMutation(resultLogData);

      toast.success(correctAnswers);
      navigate(`/Success`);
    } catch (error) {
      console.log('result log error:', error);
    }
  };
